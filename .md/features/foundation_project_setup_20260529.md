...
- 상태: in_progress
- 현재 진행 중: 초기 세팅 변경분을 의미 단위 커밋으로 분리 중.
...
| Commit | 의미 단위 | 검증 | 비고 |
|---|---|---|---|
| 예정 | 프로젝트 작업 규칙과 GitHub 워크플로 문서화 | 문서 검토 | 현재 작업에서 커밋 예정 |
| 예정 | React/Vite/Vitest 기반 세팅 | `npm test`, `npm run build` | 현재 작업에서 커밋 예정 |
| 예정 | 앱 셸 smoke test와 최소 구현 | `npm test`, `npm run build` | 현재 작업에서 커밋 예정 |

## 리뷰 장부

| Provider | 요청 방식 | 결과 | 적용/보류 판단 | 근거 |
|---|---|---|---|---|
| 예정 | draft PR 생성 후 `@codex review` | 대기 | 대기 | 초기 세팅 push/PR 후 진행 |
