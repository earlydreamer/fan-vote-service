...
- 금지: 실존 스타/작품 이미지, 공식성 오인 문구, 베이지 SaaS 히어로, 다크 스트리밍 클론, 보라/파랑 그라디언트 중심 디자인, 하드 보더/블록 포스터풍 UI, `Inter` primary font
...
방은 지속되는 팬 공간, 투표는 방 안의 현재 세션이라는 위계를 화면에서 분리하기 위해 상세 탭과 회원 상태 목업을 추가했다.

- `src/features/auth/AuthPage.tsx`: 게스트 사용자의 회원가입/로그인 진입점과 데모 계정 시작 CTA
- `src/features/auth/AuthPage.test.tsx`: 회원가입 heading, 데모 계정 CTA, 홈 피드 둘러보기 링크 테스트
- `src/features/rooms/RoomDetailPage.tsx`: `현재 투표`, `팬월`, `미션`, `결과/기록` 탭 구조와 방장/게스트 상태 표시
- `src/features/rooms/RoomDetailPage.test.tsx`: 탭 전환, 방장 관리 액션, 게스트 로그인 CTA 테스트
